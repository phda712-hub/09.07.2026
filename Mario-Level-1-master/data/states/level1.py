...
class Level1(tools._State):
    ...
    def set_game_info_values(self):
        ...
        if self.persist[c.LIVES] == 0:
            ...
            if self.game_info.get(c.LEVEL_NUM, 1) == 1 \
                    and self.mario.rect.x > 3670 \
    ...
    def end_game(self):
        """End the level.  Advances to the next level if there is one,
        otherwise the game is finished and returns to the game over screen."""
        if self.flag_timer == 0:
            self.flag_timer = self.current_time
        elif (self.current_time - self.flag_timer) > 2000:
            self.set_game_info_values()
            self.sound_manager.stop_music()
            current_level = self.game_info.get(c.LEVEL_NUM, 1)
            if current_level < c.TOTAL_LEVELS:
                self.game_info[c.LEVEL_NUM] = current_level + 1
                self.game_info[c.CAMERA_START_X] = 0
                self.next = c.LOAD_SCREEN
            else:
                self.next = c.GAME_OVER
